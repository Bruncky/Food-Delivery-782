...
describe "OrdersController", :_order do
  ...
  let(:orders_repository) { OrdersRepository.new(orders_csv_path, meals_repository, customers_repository, employees_repository) }
  ...
  it "should be initialized with 4 repository instances" do
    controller = OrdersController.new(meals_repository, customers_repository, employees_repository, orders_repository)
  ...
  describe "#add" do
    it "should ask the user for a meal index, a customer index and an employee index to be assigned" do
      ...
      expect(orders_repository.undelivered_orders[3].meal.name).to eq("Capricciosa")
      expect(orders_repository.undelivered_orders[3].employee.username).to eq("ringo")
  ...
  describe "#list_undelivered_orders" do
    it "should list undelivered orders (with meal, employee assigned and customer info)" do
      ...
      orders.drop(2).each do |order|
        expect(STDOUT).to receive(:puts).with(/#{customers_repository.find(order[3]).name}/)
      end
  ...
  describe "#list_my_orders" do
    ...
    it "should list Ringo's undelivered orders" do
      controller = OrdersController.new(meals_repository, customers_repository, employees_repository, orders_repository)
      ringo = employees_repository.find(3)  # ringo is a rider
  ...
  describe "#mark_as_delivered" do
    ...
    it "should ask the rider for an order index, mark it as delivered, and save the relevant data to the CSV file" do
      ...
      # Reload from CSV
      new_orders_repository = OrdersRepository.new(orders_csv_path, meals_repository, customers_repository, employees_repository)
      expect(new_orders_repository.undelivered_orders.map(&:id)).not_to include(4)
    end
  end
end
